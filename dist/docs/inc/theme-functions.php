...
if ( ! function_exists( 'docs_the_posts_navigation' ) ) {
	function docs_the_posts_navigation() {
		...
		the_posts_pagination(
			array(
				'mid_size' => 2,
				'prev_text' => sprintf( '<span class="nav-prev-text">%s</span>', __( 'Newer posts', 'docs' ) ),
				'next_text' => sprintf( '<span class="nav-next-text">%s</span>', __( 'Older posts', 'docs' ) ),
			)
		);
	...
	function docs_get_comment_navigation() {
		...
		if ( get_comment_pages_count() > 1 ) :
			...
			if ( get_previous_comments_link() ) {
				$output .= get_previous_comments_link( esc_html__( 'Prev Page', 'docs' ) );
			}
			if ( get_next_comments_link() ) {
				$output .= get_next_comments_link( esc_html__( 'Next Page', 'docs' ) );
			}
			$output .= '</nav>';
